...
//-----------------------------------------------------------------------------------------------//
///////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////               Class :SingleSourceSPAlg           ////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////
//-----------------------------------------------------------------------------------------------//

/**
 * SingleSourceSPAlg: is a subclass of ShortestPathAlgorithm to compute the
...
public class SingleSourceSPAlg extends ShortestPathAlgorithm {
    ...
    //Decleare verDistance array to store the distance from the source to the vertex
    ...
    public void computeDijkstraAlg() {
        ...
        for (int i = 0; i < verticesNum; i++) {
            //intially set the v=-1 and the min distance as infinity until find vertex with minimum distance
            int v = -1;
            int minDist = INFINITY;
            //find the minimum verDistance among all the vertices
            for (int j = 0; j < verticesNum; j++) {
                //check the vertex is not visited before and distance is less than the minimum distance so far
            ...
            for (int u = 0; u < verticesNum; u++) {
                ...
                if (graph.vertices[u].isVisited != true && graph.adjMatrix[v][u] != null) {
                    //check if the distance from the source to v + the distance of the edge between v + u 
                    //is less than the distance from source to vertex u
                    if (verDistance[v] + graph.adjMatrix[v][u].weight < verDistance[u]) {
                        //put the distance from the source to v + the distance of the edge between v + u as distance of u
